...
import { Bell, RefreshCw, Search, Star, ArrowRight, TrendingDown, TrendingUp, Plus, Trash2 } from "lucide-react";
...
const COLOR_CLASSES = ["bg-yellow-400", "bg-lime-500", "bg-emerald-400", "bg-sky-400", "bg-orange-400", "bg-fuchsia-400"];
...
function makeWatchlistItem(symbol, name = "") {
  const cleanSymbol = String(symbol || "").trim().toUpperCase();
  const seed = cleanSymbol.split("").reduce((sum, ch) => sum + ch.charCodeAt(0), 0);
  return {
    symbol: cleanSymbol,
    name: String(name || "").trim() || cleanSymbol,
    icon: cleanSymbol === "XAUUSD" ? "◆" : cleanSymbol.charAt(0) || "?",
    cls: cleanSymbol === "XAUUSD" ? "bg-yellow-400" : COLOR_CLASSES[seed % COLOR_CLASSES.length],
  };
}
...
function loadWatchlist() {
  ...
  try {
    ...
    return stored
      .map((item) => makeWatchlistItem(item.symbol, item.name))
...
function MobileCard({ row, open, remove }) {
  return <div className="rounded-3xl border border-yellow-400/20 bg-white/[0.03] p-5 md:hidden"><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3"><div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${row.cls} font-black text-black`}>{row.icon}</div><div><div className="text-xl font-bold">{row.symbol} <span className="text-slate-400">· {tfLabel[row.timeframe]}</span></div><div className="text-sm text-slate-400">{row.name}</div></div></div>{pill(row)}</div><div className={`mt-3 font-semibold ${regimeCls(row.regime)}`}>{row.regime} Regime •</div><div className="mt-5 grid grid-cols-4 gap-3 text-sm"><div><div className="text-slate-400">Bull</div><div className="mt-1 text-lg font-bold text-lime-300">{pct(row.bull)}</div></div><div><div className="text-slate-400">Bear</div><div className="mt-1 text-lg font-bold text-red-300">{pct(row.bear)}</div></div><div><div className="text-slate-400">Edge</div><div className={`mt-1 text-lg font-bold ${row.edge >= 0 ? "text-lime-300" : "text-red-300"}`}>{pct(row.edge)}</div></div><div><div className="text-slate-400">Quality</div><div className="mt-1"><Quality score={row.quality} /></div></div></div><div className="mt-5 flex items-center justify-between gap-3 border-t border-white/10 pt-4 text-sm"><span className="text-slate-400">Last {money(row.close)}</span><div className="flex items-center gap-4"><button onClick={() => remove(row.symbol)} className="inline-flex items-center gap-2 font-semibold text-red-300"><Trash2 className="h-4 w-4" /> Remove</button><button onClick={() => open(row)} className="inline-flex items-center gap-2 font-semibold text-yellow-300">Open Analysis <ArrowRight className="h-4 w-4" /></button></div></div></div>;
}

function DesktopTable({ rows, open, remove }) {
  return <div className="hidden overflow-hidden rounded-3xl border border-yellow-400/20 bg-white/[0.03] md:block"><div className="grid grid-cols-[1.6fr_.7fr_1fr_1.1fr_.7fr_.7fr_.7fr_.8fr_.8fr_1.2fr] border-b border-white/10 px-5 py-4 text-sm text-slate-400"><div>Asset</div><div>TF</div><div>Regime</div><div>Alert</div><div>Bull</div><div>Bear</div><div>Edge</div><div>Quality</div><div>Last</div><div>Action</div></div>{rows.map((row) => <div key={row.id} className="grid grid-cols-[1.6fr_.7fr_1fr_1.1fr_.7fr_.7fr_.7fr_.8fr_.8fr_1.2fr] items-center border-b border-white/10 px-5 py-4 text-sm last:border-b-0"><div className="flex items-center gap-3"><Star className="h-4 w-4 text-yellow-300" /><div className={`flex h-10 w-10 items-center justify-center rounded-2xl ${row.cls} font-black text-black`}>{row.icon}</div><div><div className="font-bold text-white">{row.symbol}</div><div className="text-xs text-slate-400">{row.name}</div></div></div><div>{tfLabel[row.timeframe]}</div><div className={regimeCls(row.regime)}>{row.regime} •</div><div>{pill(row)}</div><div className="font-bold text-lime-300">{pct(row.bull)}</div><div className="font-bold text-red-300">{pct(row.bear)}</div><div className={`font-bold ${row.edge >= 0 ? "text-lime-300" : "text-red-300"}`}>{pct(row.edge)}</div><div><Quality score={row.quality} /></div><div>{money(row.close)}</div><div className="flex items-center gap-4"><button onClick={() => open(row)} className="inline-flex items-center gap-2 font-semibold text-yellow-300">Open <ArrowRight className="h-4 w-4" /></button><button onClick={() => remove(row.symbol)} className="inline-flex items-center gap-2 font-semibold text-red-300"><Trash2 className="h-4 w-4" /></button></div></div>)}</div>;
}
...
export default function WatchlistPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") window.localStorage.setItem(WATCHLIST_STORAGE_KEY, JSON.stringify(watchlist));
  }, [watchlist]);
  ...
  useEffect(() => { refresh(); }, [timeframe, watchlist]);

  function addTicker(event) {
    event.preventDefault();
    const item = makeWatchlistItem(newSymbol, newName);
    if (!item.symbol) {
      setFormError("Enter a ticker symbol first.");
      return;
    }
    if (watchlist.some((existing) => existing.symbol === item.symbol)) {
      setFormError(`${item.symbol} is already on your watchlist.`);
      return;
    }
  ...
  return <div className="min-h-screen bg-[#070B0A] p-4 pb-40 text-white md:p-8 md:pb-12"><div className="mx-auto max-w-7xl space-y-6"><div className="flex items-center justify-between border-b border-white/10 pb-5"><FlowStateLogo variant="full" className="h-16 w-auto" /><Button onClick={refresh} disabled={loading} className="rounded-xl bg-emerald-500 text-slate-950 hover:bg-emerald-400"><RefreshCw className={`mr-2 h-4 w-4 ${loading ? "animate-spin" : ""}`} /> Refresh</Button></div><div className="grid gap-5 lg:grid-cols-[1fr_1.4fr] lg:items-end"><div><h1 className="text-4xl font-bold tracking-tight md:text-5xl">Watchlist</h1><p className="mt-2 text-slate-400">See what deserves attention now. Add or remove tickers anytime.</p></div><div className="grid grid-cols-3 gap-3"><Summary title="Alerts Now" value={summary.alerts} subtitle="Needs attention"><Bell className="h-5 w-5" /></Summary><Summary title="Watch Long" value={summary.long} subtitle="Bullish setups" cls="border-lime-400/25"><TrendingUp className="h-5 w-5" /></Summary><Summary title="Watch Short" value={summary.short} subtitle="Bearish setups" cls="border-red-400/25"><TrendingDown className="h-5 w-5" /></Summary></div></div><div className="rounded-3xl border border-yellow-400/20 bg-white/[0.03] p-4"><form onSubmit={addTicker} className="mb-4 grid gap-3 border-b border-white/10 pb-4 md:grid-cols-[1fr_1.4fr_auto]"><input value={newSymbol} onChange={(e) => { setNewSymbol(e.target.value); setFormError(""); }} placeholder="Ticker, e.g. AAPL or BTC-USD" className="rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm uppercase outline-none focus:border-yellow-400/50" /><input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Display name optional" className="rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm outline-none focus:border-yellow-400/50" /><button type="submit" className="inline-flex items-center justify-center gap-2 rounded-2xl border border-yellow-400/30 bg-yellow-400/15 px-5 py-3 text-sm font-bold text-yellow-200 hover:bg-yellow-400/20"><Plus className="h-4 w-4" /> Add ticker</button>{formError ? <div className="text-sm text-red-300 md:col-span-3">{formError}</div> : null}</form><div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div className="relative w-full md:max-w-sm"><Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search symbols..." className="w-full rounded-2xl border border-white/10 bg-black/30 py-3 pl-12 pr-4 text-sm outline-none focus:border-yellow-400/50" /></div><div className="flex flex-wrap gap-2">{TIMEFRAME_OPTIONS.map((tf) => <button key={tf} onClick={() => setTimeframe(tf)} className={`rounded-xl border px-5 py-3 text-sm font-semibold ${timeframe === tf ? "border-yellow-400 bg-yellow-400/15 text-yellow-300" : "border-white/10 bg-white/5 text-slate-300"}`}>{tfLabel[tf]}</button>)}{[["all", "All"], ["alerts", "Alerts"], ["ready", "Ready"]].map(([key, label]) => <button key={key} onClick={() => setFilter(key)} className={`rounded-xl border px-5 py-3 text-sm font-semibold ${filter === key ? "border-yellow-400 bg-yellow-400/15 text-yellow-300" : "border-white/10 bg-white/5 text-slate-300"}`}>{label}</button>)}</div></div></div>{watchlist.length === 0 ? <div className="rounded-3xl border border-dashed border-yellow-400/25 bg-white/[0.03] p-8 text-center text-slate-300">Your watchlist is empty. Add a ticker above to start scanning.</div> : null}<DesktopTable rows={filtered} open={open} remove={removeTicker} /><div className="space-y-3 md:hidden">{filtered.map((row) => <MobileCard key={row.id} row={row} open={open} remove={removeTicker} />)}</div></div></div>;
}
