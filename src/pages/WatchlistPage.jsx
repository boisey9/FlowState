...
export default function WatchlistPage() {
  ...
  async function fetchRow(item, timeframe) {
    if (!SUPABASE_ANON_KEY) return normalize(item, timeframe, null, "Missing anon key");
    try {
      const res = await fetch(FUNCTION_URL, { method: "POST", headers: { "Content-Type": "application/json", apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${SUPABASE_ANON_KEY}` }, body: JSON.stringify({ symbol: item.symbol, timeframe, execution_confirmed: false, outputsize: 5000 }) });
      const data = await res.json();
      if (!res.ok || data.error) throw new Error(data.error || `HTTP ${res.status}`);
      return normalize(item, timeframe, { ...data, symbol: item.symbol, timeframe });
    } catch (err) { return normalize(item, timeframe, null, err instanceof Error ? err.message : String(err)); }
  }
  async function refresh() {
    setLoading(true);
    const jobs = WATCHLIST.flatMap((item) => TIMEFRAMES.map((tf) => fetchRow(item, tf)));
  ...
  return <div className="min-h-screen bg-[#070B0A] p-4 pb-28 text-white md:p-8"><div className="mx-auto max-w-7xl space-y-6"><div className="flex items-center justify-between border-b border-white/10 pb-5"><FlowStateLogo variant="full" className="h-16 w-auto" /><div className="hidden items-center gap-8 md:flex"><button onClick={() => { window.location.hash = "#analyze"; }} className="text-slate-300 hover:text-yellow-300">Analyze</button><span className="border-b-2 border-yellow-400 pb-2 font-semibold text-yellow-300">Watchlist</span><span className="text-slate-300">Alerts</span></div><Button onClick={refresh} disabled={loading} className="rounded-xl bg-emerald-500 text-slate-950 hover:bg-emerald-400"><RefreshCw className={`mr-2 h-4 w-4 ${loading ? "animate-spin" : ""}`} /> Refresh</Button></div><div className="grid gap-5 lg:grid-cols-[1fr_1.4fr] lg:items-end"><div><h1 className="text-4xl font-bold tracking-tight md:text-5xl">Watchlist</h1><p className="mt-2 text-slate-400">See what deserves attention now.</p></div><div className="grid grid-cols-3 gap-3"><Summary title="Alerts Now" value={summary.alerts} subtitle="Needs attention"><Bell className="h-5 w-5" /></Summary><Summary title="Watch Long" value={summary.long} subtitle="Bullish setups" cls="border-lime-400/25"><TrendingUp className="h-5 w-5" /></Summary><Summary title="Watch Short" value={summary.short} subtitle="Bearish setups" cls="border-red-400/25"><TrendingDown className="h-5 w-5" /></Summary></div></div><div className="rounded-3xl border border-yellow-400/20 bg-white/[0.03] p-4"><div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div className="relative w-full md:max-w-sm"><Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search symbols..." className="w-full rounded-2xl border border-white/10 bg-black/30 py-3 pl-12 pr-4 text-sm outline-none focus:border-yellow-400/50" /></div><div className="flex flex-wrap gap-2">{[["all", "All"], ["alerts", "Alerts"], ["ready", "Ready"]].map(([key, label]) => <button key={key} onClick={() => setFilter(key)} className={`rounded-xl border px-5 py-3 text-sm font-semibold ${filter === key ? "border-yellow-400 bg-yellow-400/15 text-yellow-300" : "border-white/10 bg-white/5 text-slate-300"}`}>{label}</button>)}</div></div></div><DesktopTable rows={filtered} open={open} /><div className="space-y-3 md:hidden">{filtered.map((row) => <MobileCard key={row.id} row={row} open={open} />)}</div><div className="fixed bottom-4 left-4 right-4 z-20 grid grid-cols-3 rounded-3xl border border-yellow-400/20 bg-black/90 p-2 text-center text-xs backdrop-blur md:hidden"><button onClick={() => { window.location.hash = "#analyze"; }} className="rounded-2xl p-3 text-slate-400">Analyze</button><button className="rounded-2xl border border-yellow-400/35 bg-yellow-400/10 p-3 text-yellow-300"><Star className="mx-auto mb-1 h-5 w-5" />Watchlist</button><button className="rounded-2xl p-3 text-slate-400"><Bell className="mx-auto mb-1 h-5 w-5" />Alerts</button></div></div></div>;
}
